...
public class ArgumentResolverTest {
    ...
    public void resolve_shouldReturnListOfResolvedArgs(String[] testArgs, List<Argument> expected) {
        List<Argument> actual = new ArgumentResolver().resolve(testArgs);
    ...
    public void resolve_shouldThrowExceptionWhenArgsSizeLessThan2() {
        assertThatThrownBy(() -> new ArgumentResolver().resolve(new String[] {"--path"})).isInstanceOf(
    ...
    public void resolve_shouldThrowExceptionWhenFirstParameterIsNotPath() {
        assertThatThrownBy(() -> new ArgumentResolver().resolve(new String[] {"--format"})).isInstanceOf(
            IllegalArgumentException.class);
    }
    ...
    public void resolve_shouldThrowExceptionWhenSomeParameterHasNoInfo() {
        assertThatThrownBy(() -> new ArgumentResolver().resolve(new String[] {"--path", "text.txt",
            "--format"})).isInstanceOf(
            IllegalArgumentException.class);
    }
}
